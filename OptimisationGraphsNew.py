...
Lmes17=np.delete(Lmes17, np.where(FillNumber17==6160)[0])
...
#loading data --- ONLINE
f=open('OnlineNew/OptimalFill{}.txt'.format(year),"r")
lines=f.readlines()
t_online=[]
for x in lines:
...
def TimePlots():
    ...
    plt.savefig('OnlineNew/ViolinPlot{}.pdf'.format(year))
    #plt.show()
...
def LumiPlots(Lint_online, Ltot_online, Lint_num, Ltot_num):
    """Function that produces the violin plots for the luminoisties of the actual case and online and numerical ones.
    """
    ...
    plt.savefig('OnlineNew/ViolinPlot{}_Luminosity.pdf'.format(year))
    #plt.show()
