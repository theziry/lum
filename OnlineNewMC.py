...
# with MC algorithm for missing fills                                        #
##############################################################################
...
def MonteCarlo(t_optimal, tta_opt, check=False):
    """Function that simulates the fill and turn around times when needed, 
    and eventually check the new distributions."

    Args:
        t_optimal (narray): Optimal fill times
        tta_opt (narray): Optimal turn around times

    Returns:
        new_tf: simulated fill times
        new_tta: simulated turn around times
    """
    new_tf=np.random.choice(t_optimal, size=1)
    new_tta=np.random.choice(tta_opt, size=1)
    
    if check==True:
        test1=stats.ks_2samp(t_optimal, new_tf)
        test2=stats.ks_2samp(tta_opt, new_tta)
        if test1[1]>0.05 and test2[1]>0.05:
            print('The new samples are distributed according to the occurred tta and tf.')
        elif test1[1]<0.05 and test2[1]<0.05:
            print('WANING! The new samples are NOT distributed according to the occurred tta and tf.')
        elif test1[1]<0.05 and test2[1]>0.05:
            print('WANING! The tf are NOT distributed according the occurred ones.')
        elif test1[1]>0.05 and test2[1]<0.05:
            print('WANING! The tta are NOT distributed according the occurred ones.')
    
    return new_tf, new_tta
